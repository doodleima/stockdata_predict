...
def headline_crawler(jongmok) :
    ...
    print("=== " + str(jongmok) + "에 대한 1주간 기사 헤드라인 크롤링 시작 ===")
    for num in range(0, 400, 10): # 50개의 페이지에 대해 수행, 30개가 적당할지도? 긁어올 페이지 수 고려해봐야..
    ...
    print("=== " + str(jongmok) + "에 대한 1주간 기사 헤드라인 크롤링 완료 ===")

    df = pd.DataFrame(wordslist)
    df.columns = ['헤드라인']
    df.to_csv(".\\헤드라인\\" + str(jongmok) + " 기사 헤드라인.csv", encoding = 'UTF-8-SIG')
