...
# 훈련 데이터
X_train = np.load(open('..\\word_dir(headline)\\headline_input.npy', 'rb'))
Y_train = np.load(open('..\\word_dir(headline)\\headline_label.npy', 'rb'))

# 테스트 데이터
X_test = np.load(open('..\\word_dir\\test_data.npy', 'rb'))
Y_test = np.load(open('..\\word_dir\\test_label.npy', 'rb'))

data_configs = json.load(open('..\\word_dir(headline)\\data_configs.json', 'r', -1, "UTF-8-SIG"))
...
hist = model.fit(X_train, Y_train, batch_size = 4, epochs = 10, validation_split=0.25, callbacks = [ckpoint, erstoping])#ckpoint)
