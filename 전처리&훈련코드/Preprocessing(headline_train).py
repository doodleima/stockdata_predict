...
total_data = pd.read_csv('..\\훈련데이터(NSMC&Headline)\\훈련데이터(헤드라인종합)_레이블재검토.csv', encoding ='UTF-8-SIG')

del total_data['Unnamed: 0'] # 불필요 컬럼 제거(이전 index)
total_data = total_data.astype({'레이블' : 'int'})
...
os.makedirs('..\\word_dir(headline)', exist_ok= True)
np.save(open('..\\word_dir(headline)\\headline_input.npy', 'wb'), X_train)
np.save(open('..\\word_dir(headline)\\headline_label.npy', 'wb'), Y_train)

json.dump(data_configs, open('..\\word_dir(headline)\\data_configs.json', 'w', -1, "UTF-8-SIG"), ensure_ascii = False)
