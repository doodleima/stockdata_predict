...
def get_market_data(jongmok) :
    jongmoklist = ['Dividends','Stock Splits']
    ...
    print("===== " + jongmok + "(" + jongmok_code + ")의 주가 데이터(5Y)를 불러옵니다. =====")
    # yahoo finance 라이브러리를 사용하여 해당 종목코드에 대한 5년간의 주가 데이터 불러옴
    market_data = yf.Ticker(jongmok_code + ".KS")
    jongmok_history = market_data.history(period='5Y')

    for i in jongmoklist : # 반복문 돌려 필요 없는 컬럼은 제거
        del jongmok_history[i]
    print("===== " + jongmok + "(" + jongmok_code + ")의 주가 데이터(5Y)를 불러오기 완료 =====")
